...
def polarity_func(attributions, polarity):
...
def compute_threshold_by_top_percent(attributions,
                                    percentage=60,
...
def linear_transform(attributions,
    ...
    m = compute_threshold_by_top_percent(attributions,
                                      percentage=100-clip_above_percentile,
                                      plot_distribution=plot_distribution)
    e = compute_threshold_by_top_percent(attributions,
...
def outlines_func(attributions, percentage=90,
...
def overlay_func(attributions, image):
...
def visualize(attributions,
    ...
    if polarity == 'both':
        pos_attributions = visualize(
                                    ...
                                    outlines=outlines,
                                    outlines_percent=outlines_percent,
                                    overlay=False,
                                    plot_distribution=plot_distribution)
        ...
        if overlay:
            attributions = overlay_func(attributions, image)
    
        return attributions
    elif polarity == 'positive':
        attributions = polarity_func(attributions, polarity=polarity)
    ...
    attributions = convert_to_grayscale(attributions)
  
    attributions = linear_transform(attributions,
